...
def ned_to_gate(autonav, ned_x2, ned_y2):
        '''
        @name: ned_to_gate
        ...
        @return: gate_x2: target x coordinate in gate reference frame
                 gate_y2: target y coordinate in gate reference frame
        '''
        p = np.array([ned_x2-autonav.ned_channel_origin_x, ned_y2-autonav.ned_channel_origin_y])
        J = autonav.rotation_matrix(autonav.ned_alpha)
...
class AutoNav:
    ...
    def center_point(self):
        ...
        xm, ym = gate_to_body(5,0,alpha,xc,yc)
    ...
    def farther(self, dist):
        ...
        @return: --
        '''
        self.target_x, self.target_y = gate_to_ned(dist, 0, self.ned_alpha, self.target_x, self.target_y)
        add_path(self, [self.target_x, self.target_y, 0])

    def buoy_waypoints(self, init_x, init_y, buoy_x,buoy_y):
        '''
        @name: buoy_waypoints
        ...
        w1_x, w1_y = body_to_ned(self, w1[0], w1[1])
        w2_x, w2_y = body_to_ned(self, w2[0], w2[1])
        w3_x, w3_y = body_to_ned(self, w3[0], w3[1])
        w4_x, w4_y = gate_to_ned(3, 0, self.ned_alpha, init_x, init_y)
        path_array.data = [w1_x, w1_y, w2_x, w2_y, w3_x, w3_y,
                          w4_x, w4_y, init_x, init_y, 0]
